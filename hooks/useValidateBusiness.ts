...
type MutationInput =
  ValidateBusinessOp['requestBody']['content']['application/json'];
type MutationResponse = ValidateBusinessOp['responses'][200]['content']['*/*'];

type SignupBusinessOp = operations['signupBusiness'];
type SignupBusinessInput =
  SignupBusinessOp['requestBody']['content']['application/json'];
type SignupBusinessResponse =
  SignupBusinessOp['responses'][200]['content']['*/*'];

export function useValidateBusiness() {
  return useMutation({
    mutationFn: async (request: MutationInput) => {
      // fetchClient에 추출한 Response 타입 인자 주입
      const res = await fetchClient<MutationResponse>(
...
export function useSignupBusiness() {
  return useMutation({
    mutationFn: async (request: SignupBusinessInput) => {
      const res = await fetchClient<SignupBusinessResponse>(
        '/api/signup/business',
        {
          method: 'POST',
          body: JSON.stringify(request),
      ...
      if (!res.isSuccess) {
        throw new Error(res.message || '회원가입 처리 중 오류가 발생했습니다.');
      }
