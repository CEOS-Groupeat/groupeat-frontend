...
interface ApiResponseOwnerTermDetail {
  isSuccess: boolean;
  code: string;
  message: string;
  data: OwnerTermDetailDTO;
}

export default function OwnerTermInfoPage() {
  ...
  const { data: termDetail, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetchClient<ApiResponseOwnerTermDetail>(
